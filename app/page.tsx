  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#0A0612] text-zinc-100">
      <style>{`
        ...
        @media (prefers-reduced-motion: reduce) { .rise, .anim { animation: none !important; } }
      `}</style>
      ...
      <section className="relative mx-auto flex min-h-[calc(100vh-73px)] max-w-7xl flex-col items-center gap-10 px-6 pt-16 pb-12 md:flex-row md:gap-4 md:pt-8">
        ...
        <div className="flex flex-col gap-6 md:w-[45%]">
          <span className="rise text-xs tracking-[0.3em] text-cyan-400/80 font-[family-name:var(--font-space-mono)]">
            ◍ RESPAWND // GAME TRACKER
          </span>
          <h1 className="rise text-6xl font-bold leading-[1.02] tracking-tight md:text-7xl font-[family-name:var(--font-display)]" style={{ animationDelay: '0.05s' }}>
            Your games.
            <br />
        ...
        <div className="relative flex h-[420px] w-full items-center justify-center md:h-[600px] md:w-[55%]">
          {/* halo group — centered on controller at 46% */}
          ...
          <div className="anim pointer-events-none absolute left-1/2 top-[32%] h-80 w-80 rounded-full border-2 border-violet-400/50 md:h-[420px] md:w-[420px]"
          ...
          <div className="anim pointer-events-none absolute left-1/2 top-[32%] h-64 w-64 rounded-full border border-cyan-400/40 md:h-80 md:w-80"
               style={{ animation: 'ringSpin 18s linear infinite' }} />
          <div className="anim pointer-events-none absolute left-1/2 top-[32%] h-[360px] w-[360px] rounded-full border border-fuchsia-400/20 md:h-[480px] md:w-[480px]"
      ...
      <section className="relative px-6 pb-24 pt-8">
        <div className="mx-auto grid max-w-5xl grid-cols-1 gap-6 sm:grid-cols-3">
          {FEATURES.map(({ tag, icon, title, desc }, i) => (
            <div
              key={title}
              className="rise rounded-2xl border border-white/10 bg-white/[0.03] p-6 backdrop-blur-sm transition duration-300 hover:-translate-y-1 hover:border-violet-500/40 hover:bg-white/[0.06] hover:shadow-[0_0_30px_-10px_rgba(124,58,237,0.6)]"
